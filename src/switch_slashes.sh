...
#       if not specified the direction is set to invert
...
main() {
    if [ $# -eq 0 ] || [ $# -gt 2 ]; then
        ...
        echo "       [<direction>] - three options possible: a) left_to_right b) right_to_left c) invert"
        echo "       if not specified the direction is set to invert"
